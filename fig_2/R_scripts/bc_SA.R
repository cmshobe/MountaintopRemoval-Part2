...
bc_pre_SA <- scan('../flat_inputs/bc_pre_SA.txt')
bc_post_SA <- scan('../flat_inputs/bc_post_SA.txt')

pre_name <- 'bc_pre_SA'
post_name <- 'bc_post_SA'

bc_SA <- data.frame(bc_pre_SA, bc_post_SA)
names(bc_SA) <- c(pre_name,post_name)
...
set.seed(123456)
random_subsample <- bc_SA[sample(nrow(bc_SA), round(length(bc_pre_SA)*pro_measurements,0)), ]

x <- random_subsample$bc_pre_SA
y <- random_subsample$bc_post_SA
...
bayes_factor_bc_SA <- computeBayesFactorOneZero(signedRankSamples$deltaSamples, 
                                                   whichTest = "Wilcoxon",
                                                   priorParameter = 1 / sqrt(2))
#calculate 95% and 99% HPDI (highest posterior density interval) 
cred_interval_99_bc_SA <- hdi(signedRankSamples$deltaSamples, 0.99)
cred_interval_95_bc_SA <- hdi(signedRankSamples$deltaSamples, 0.95)
...
values_column <- c(posterior_median,
                   bayes_factor_bc_SA,
                   cred_interval_99_bc_SA[1],
                   cred_interval_99_bc_SA[2],
                   cred_interval_95_bc_SA[1],
                   cred_interval_95_bc_SA[2])

#build dataframe
export_df <- data.frame(labels_column, values_column)
write.csv(export_df, '../R_outputs/sam_bc_SA.csv', row.names = FALSE)
write.csv(signedRankSamples$deltaSamples, '../R_outputs/sam_bc_SA_samples.csv', row.names = FALSE)

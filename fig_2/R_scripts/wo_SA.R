...
wo_pre_SA <- scan('../flat_inputs/wo_pre_SA.txt')
wo_post_SA <- scan('../flat_inputs/wo_post_SA.txt')

pre_name <- 'wo_pre_SA'
post_name <- 'wo_post_SA'

wo_SA <- data.frame(wo_pre_SA, wo_post_SA)
names(wo_SA) <- c(pre_name,post_name)
...
set.seed(123456)
random_subsample <- wo_SA[sample(nrow(wo_SA), round(length(wo_pre_SA)*pro_measurements,0)), ]

x <- random_subsample$wo_pre_SA
y <- random_subsample$wo_post_SA
...
bayes_factor_wo_SA <- computeBayesFactorOneZero(signedRankSamples$deltaSamples, 
                                                   whichTest = "Wilcoxon",
                                                   priorParameter = 1 / sqrt(2))
#calculate 95% and 99% HPDI (highest posterior density interval) 
cred_interval_99_wo_SA <- hdi(signedRankSamples$deltaSamples, 0.99)
cred_interval_95_wo_SA <- hdi(signedRankSamples$deltaSamples, 0.95)
...
values_column <- c(posterior_median,
                   bayes_factor_wo_SA,
                   cred_interval_99_wo_SA[1],
                   cred_interval_99_wo_SA[2],
                   cred_interval_95_wo_SA[1],
                   cred_interval_95_wo_SA[2])

#build dataframe
export_df <- data.frame(labels_column, values_column)
write.csv(export_df, '../R_outputs/sam_wo_SA.csv', row.names = FALSE)
write.csv(signedRankSamples$deltaSamples, '../R_outputs/sam_wo_SA_samples.csv', row.names = FALSE)

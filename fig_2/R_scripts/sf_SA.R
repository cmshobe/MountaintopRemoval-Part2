...
sf_pre_SA <- scan('../flat_inputs/sf_pre_SA.txt')
sf_post_SA <- scan('../flat_inputs/sf_post_SA.txt')

pre_name <- 'sf_pre_SA'
post_name <- 'sf_post_SA'

sf_SA <- data.frame(sf_pre_SA, sf_post_SA)
names(sf_SA) <- c(pre_name,post_name)
...
set.seed(123456)
random_subsample <- sf_SA[sample(nrow(sf_SA), round(length(sf_pre_SA)*pro_measurements,0)), ]

x <- random_subsample$sf_pre_SA
y <- random_subsample$sf_post_SA
...
bayes_factor_sf_SA <- computeBayesFactorOneZero(signedRankSamples$deltaSamples, 
                                                   whichTest = "Wilcoxon",
                                                   priorParameter = 1 / sqrt(2))
#calculate 95% and 99% HPDI (highest posterior density interval) 
cred_interval_99_sf_SA <- hdi(signedRankSamples$deltaSamples, 0.99)
cred_interval_95_sf_SA <- hdi(signedRankSamples$deltaSamples, 0.95)
...
values_column <- c(posterior_median,
                   bayes_factor_sf_SA,
                   cred_interval_99_sf_SA[1],
                   cred_interval_99_sf_SA[2],
                   cred_interval_95_sf_SA[1],
                   cred_interval_95_sf_SA[2])

#build dataframe
export_df <- data.frame(labels_column, values_column)
write.csv(export_df, '../R_outputs/sam_sf_SA.csv', row.names = FALSE)
write.csv(signedRankSamples$deltaSamples, '../R_outputs/sam_sf_SA_samples.csv', row.names = FALSE)

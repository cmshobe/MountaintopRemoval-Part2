...
sf_pre_slope <- scan('../flat_inputs/sf_pre_slope.txt')
sf_post_slope <- scan('../flat_inputs/sf_post_slope.txt')

pre_name <- 'sf_pre_slope'
post_name <- 'sf_post_slope'

sf_slope <- data.frame(sf_pre_slope, sf_post_slope)
names(sf_slope) <- c(pre_name,post_name)
...
set.seed(123456)
random_subsample <- sf_slope[sample(nrow(sf_slope), round(length(sf_pre_slope)*pro_measurements,0)), ]

x <- random_subsample$sf_pre_slope
y <- random_subsample$sf_post_slope
...
bayes_factor_sf_slope <- computeBayesFactorOneZero(signedRankSamples$deltaSamples, 
                                                  whichTest = "Wilcoxon",
                                                  priorParameter = 1 / sqrt(2))
#calculate 95% and 99% HPDI (highest posterior density interval) 
cred_interval_99_sf_slope <- hdi(signedRankSamples$deltaSamples, 0.99)
cred_interval_95_sf_slope <- hdi(signedRankSamples$deltaSamples, 0.95)
...
values_column <- c(posterior_median,
                   bayes_factor_sf_slope,
                   cred_interval_99_sf_slope[1],
                   cred_interval_99_sf_slope[2],
                   cred_interval_95_sf_slope[1],
                   cred_interval_95_sf_slope[2])

#build dataframe
export_df <- data.frame(labels_column, values_column)
write.csv(export_df, '../R_outputs/sam_sf_slope.csv', row.names = FALSE)
write.csv(signedRankSamples$deltaSamples, '../R_outputs/sam_sf_slope_samples.csv', row.names = FALSE)

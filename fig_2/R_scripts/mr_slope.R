...
mr_pre_slope <- scan('../flat_inputs/mr_pre_slope.txt')
mr_post_slope <- scan('../flat_inputs/mr_post_slope.txt')

pre_name <- 'mr_pre_slope'
post_name <- 'mr_post_slope'

mr_slope <- data.frame(mr_pre_slope, mr_post_slope)
names(mr_slope) <- c(pre_name,post_name)
...
set.seed(123456)
random_subsample <- mr_slope[sample(nrow(mr_slope), round(length(mr_pre_slope)*pro_measurements,0)), ]

x <- random_subsample$mr_pre_slope
y <- random_subsample$mr_post_slope
...
bayes_factor_mr_slope <- computeBayesFactorOneZero(signedRankSamples$deltaSamples, 
                                                  whichTest = "Wilcoxon",
                                                  priorParameter = 1 / sqrt(2))
#calculate 95% and 99% HPDI (highest posterior density interval) 
cred_interval_99_mr_slope <- hdi(signedRankSamples$deltaSamples, 0.99)
cred_interval_95_mr_slope <- hdi(signedRankSamples$deltaSamples, 0.95)
...
values_column <- c(posterior_median,
                   bayes_factor_mr_slope,
                   cred_interval_99_mr_slope[1],
                   cred_interval_99_mr_slope[2],
                   cred_interval_95_mr_slope[1],
                   cred_interval_95_mr_slope[2])

#build dataframe
export_df <- data.frame(labels_column, values_column)
write.csv(export_df, '../R_outputs/sam_mr_slope.csv', row.names = FALSE)
write.csv(signedRankSamples$deltaSamples, '../R_outputs/sam_mr_slope_samples.csv', row.names = FALSE)

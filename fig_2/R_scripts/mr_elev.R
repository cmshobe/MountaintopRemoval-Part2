...
mr_pre_elev <- scan('../flat_inputs/mr_pre_elev.txt')
mr_post_elev <- scan('../flat_inputs/mr_post_elev.txt')

pre_name <- 'mr_pre_elev'
post_name <- 'mr_post_elev'

mr_elev <- data.frame(mr_pre_elev, mr_post_elev)
names(mr_elev) <- c(pre_name,post_name)
...
set.seed(123456)
random_subsample <- mr_elev[sample(nrow(mr_elev), round(length(mr_pre_elev)*pro_measurements,0)), ]

x <- random_subsample$mr_pre_elev
y <- random_subsample$mr_post_elev
...
bayes_factor_mr_elev <- computeBayesFactorOneZero(signedRankSamples$deltaSamples, 
                                                whichTest = "Wilcoxon",
                                                priorParameter = 1 / sqrt(2))
#calculate 95% and 99% HPDI (highest posterior density interval) 
cred_interval_99_mr_elev <- hdi(signedRankSamples$deltaSamples, 0.99)
cred_interval_95_mr_elev <- hdi(signedRankSamples$deltaSamples, 0.95)
...
values_column <- c(posterior_median,
                   bayes_factor_mr_elev,
                   cred_interval_99_mr_elev[1],
                   cred_interval_99_mr_elev[2],
                   cred_interval_95_mr_elev[1],
                   cred_interval_95_mr_elev[2])

#build dataframe
export_df <- data.frame(labels_column, values_column)
write.csv(export_df, '../R_outputs/sam_mr_elev.csv', row.names = FALSE)
write.csv(signedRankSamples$deltaSamples, '../R_outputs/sam_mr_elev_samples.csv', row.names = FALSE)

...
sf_pre_elev <- scan('../flat_inputs/sf_pre_elev.txt')
sf_post_elev <- scan('../flat_inputs/sf_post_elev.txt')

pre_name <- 'sf_pre_elev'
post_name <- 'sf_post_elev'

sf_elev <- data.frame(sf_pre_elev, sf_post_elev)
names(sf_elev) <- c(pre_name,post_name)
...
set.seed(123456)
random_subsample <- sf_elev[sample(nrow(sf_elev), round(length(sf_pre_elev)*pro_measurements,0)), ]

x <- random_subsample$sf_pre_elev
y <- random_subsample$sf_post_elev
...
bayes_factor_sf_elev <- computeBayesFactorOneZero(signedRankSamples$deltaSamples, 
                                                  whichTest = "Wilcoxon",
                                                  priorParameter = 1 / sqrt(2))
#calculate 95% and 99% HPDI (highest posterior density interval) 
cred_interval_99_sf_elev <- hdi(signedRankSamples$deltaSamples, 0.99)
cred_interval_95_sf_elev <- hdi(signedRankSamples$deltaSamples, 0.95)
...
values_column <- c(posterior_median,
                   bayes_factor_sf_elev,
                   cred_interval_99_sf_elev[1],
                   cred_interval_99_sf_elev[2],
                   cred_interval_95_sf_elev[1],
                   cred_interval_95_sf_elev[2])

#build dataframe
export_df <- data.frame(labels_column, values_column)
write.csv(export_df, '../R_outputs/sam_sf_elev.csv', row.names = FALSE)
write.csv(signedRankSamples$deltaSamples, '../R_outputs/sam_sf_elev_samples.csv', row.names = FALSE)

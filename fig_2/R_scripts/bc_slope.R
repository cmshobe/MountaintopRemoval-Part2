...
bc_pre_slope <- scan('../flat_inputs/bc_pre_slope.txt')
bc_post_slope <- scan('../flat_inputs/bc_post_slope.txt')

pre_name <- 'bc_pre_slope'
post_name <- 'bc_post_slope'

bc_slope <- data.frame(bc_pre_slope, bc_post_slope)
names(bc_slope) <- c(pre_name,post_name)
...
set.seed(123456)
random_subsample <- bc_slope[sample(nrow(bc_slope), round(length(bc_pre_slope)*pro_measurements,0)), ]

x <- random_subsample$bc_pre_slope
y <- random_subsample$bc_post_slope
...
bayes_factor_bc_slope <- computeBayesFactorOneZero(signedRankSamples$deltaSamples, 
                                                  whichTest = "Wilcoxon",
                                                  priorParameter = 1 / sqrt(2))
#calculate 95% and 99% HPDI (highest posterior density interval) 
cred_interval_99_bc_slope <- hdi(signedRankSamples$deltaSamples, 0.99)
cred_interval_95_bc_slope <- hdi(signedRankSamples$deltaSamples, 0.95)
...
values_column <- c(posterior_median,
                   bayes_factor_bc_slope,
                   cred_interval_99_bc_slope[1],
                   cred_interval_99_bc_slope[2],
                   cred_interval_95_bc_slope[1],
                   cred_interval_95_bc_slope[2])

#build dataframe
export_df <- data.frame(labels_column, values_column)
write.csv(export_df, '../R_outputs/sam_bc_slope.csv', row.names = FALSE)
write.csv(signedRankSamples$deltaSamples, '../R_outputs/sam_bc_slope_samples.csv', row.names = FALSE)

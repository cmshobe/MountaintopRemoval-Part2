...
mr_pre_SA <- scan('../flat_inputs/mr_pre_SA.txt')
mr_post_SA <- scan('../flat_inputs/mr_post_SA.txt')

pre_name <- 'mr_pre_SA'
post_name <- 'mr_post_SA'

mr_SA <- data.frame(mr_pre_SA, mr_post_SA)
names(mr_SA) <- c(pre_name,post_name)
...
set.seed(123456)
random_subsample <- mr_SA[sample(nrow(mr_SA), round(length(mr_pre_SA)*pro_measurements,0)), ]

x <- random_subsample$mr_pre_SA
y <- random_subsample$mr_post_SA
...
bayes_factor_mr_SA <- computeBayesFactorOneZero(signedRankSamples$deltaSamples, 
                                                   whichTest = "Wilcoxon",
                                                   priorParameter = 1 / sqrt(2))
#calculate 95% and 99% HPDI (highest posterior density interval) 
cred_interval_99_mr_SA <- hdi(signedRankSamples$deltaSamples, 0.99)
cred_interval_95_mr_SA <- hdi(signedRankSamples$deltaSamples, 0.95)
...
values_column <- c(posterior_median,
                   bayes_factor_mr_SA,
                   cred_interval_99_mr_SA[1],
                   cred_interval_99_mr_SA[2],
                   cred_interval_95_mr_SA[1],
                   cred_interval_95_mr_SA[2])

#build dataframe
export_df <- data.frame(labels_column, values_column)
write.csv(export_df, '../R_outputs/sam_mr_SA.csv', row.names = FALSE)
write.csv(signedRankSamples$deltaSamples, '../R_outputs/sam_mr_SA_samples.csv', row.names = FALSE)

...
lc_pre_slope <- scan('../flat_inputs/lc_pre_slope.txt')
lc_post_slope <- scan('../flat_inputs/lc_post_slope.txt')

pre_name <- 'lc_pre_slope'
post_name <- 'lc_post_slope'

lc_slope <- data.frame(lc_pre_slope, lc_post_slope)
names(lc_slope) <- c(pre_name,post_name)
...
set.seed(123456)
random_subsample <- lc_slope[sample(nrow(lc_slope), round(length(lc_pre_slope)*pro_measurements,0)), ]

x <- random_subsample$lc_pre_slope
y <- random_subsample$lc_post_slope
...
bayes_factor_lc_slope <- computeBayesFactorOneZero(signedRankSamples$deltaSamples, 
                                                  whichTest = "Wilcoxon",
                                                  priorParameter = 1 / sqrt(2))
#calculate 95% and 99% HPDI (highest posterior density interval) 
cred_interval_99_lc_slope <- hdi(signedRankSamples$deltaSamples, 0.99)
cred_interval_95_lc_slope <- hdi(signedRankSamples$deltaSamples, 0.95)
...
values_column <- c(posterior_median,
                   bayes_factor_lc_slope,
                   cred_interval_99_lc_slope[1],
                   cred_interval_99_lc_slope[2],
                   cred_interval_95_lc_slope[1],
                   cred_interval_95_lc_slope[2])

#build dataframe
export_df <- data.frame(labels_column, values_column)
write.csv(export_df, '../R_outputs/sam_lc_slope.csv', row.names = FALSE)
write.csv(signedRankSamples$deltaSamples, '../R_outputs/sam_lc_slope_samples.csv', row.names = FALSE)

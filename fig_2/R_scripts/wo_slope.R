...
wo_pre_slope <- scan('../flat_inputs/wo_pre_slope.txt')
wo_post_slope <- scan('../flat_inputs/wo_post_slope.txt')

pre_name <- 'wo_pre_slope'
post_name <- 'wo_post_slope'

wo_slope <- data.frame(wo_pre_slope, wo_post_slope)
names(wo_slope) <- c(pre_name,post_name)
...
set.seed(123456)
random_subsample <- wo_slope[sample(nrow(wo_slope), round(length(wo_pre_slope)*pro_measurements,0)), ]

x <- random_subsample$wo_pre_slope 
y <- random_subsample$wo_post_slope
...
bayes_factor_wo_slope <- computeBayesFactorOneZero(signedRankSamples$deltaSamples, 
                                                  whichTest = "Wilcoxon",
                                                  priorParameter = 1 / sqrt(2))
#calculate 95% and 99% HPDI (highest posterior density interval) 
cred_interval_99_wo_slope <- hdi(signedRankSamples$deltaSamples, 0.99)
cred_interval_95_wo_slope <- hdi(signedRankSamples$deltaSamples, 0.95)
...
values_column <- c(posterior_median,
                   bayes_factor_wo_slope,
                   cred_interval_99_wo_slope[1],
                   cred_interval_99_wo_slope[2],
                   cred_interval_95_wo_slope[1],
                   cred_interval_95_wo_slope[2])

#build dataframe
export_df <- data.frame(labels_column, values_column)
write.csv(export_df, '../R_outputs/sam_wo_slope.csv', row.names = FALSE)
write.csv(signedRankSamples$deltaSamples, '../R_outputs/sam_wo_slope_samples.csv', row.names = FALSE)

...
lc_pre_SA <- scan('../flat_inputs/lc_pre_SA.txt')
lc_post_SA <- scan('../flat_inputs/lc_post_SA.txt')

pre_name <- 'lc_pre_SA'
post_name <- 'lc_post_SA'

lc_SA <- data.frame(lc_pre_SA, lc_post_SA)
names(lc_SA) <- c(pre_name,post_name)
...
set.seed(123456)
random_subsample <- lc_SA[sample(nrow(lc_SA), round(length(lc_pre_SA)*pro_measurements,0)), ]

x <- random_subsample$lc_pre_SA
y <- random_subsample$lc_post_SA
...
bayes_factor_lc_SA <- computeBayesFactorOneZero(signedRankSamples$deltaSamples, 
                                                   whichTest = "Wilcoxon",
                                                   priorParameter = 1 / sqrt(2))
#calculate 95% and 99% HPDI (highest posterior density interval) 
cred_interval_99_lc_SA <- hdi(signedRankSamples$deltaSamples, 0.99)
cred_interval_95_lc_SA <- hdi(signedRankSamples$deltaSamples, 0.95)
...
values_column <- c(posterior_median,
                   bayes_factor_lc_SA,
                   cred_interval_99_lc_SA[1],
                   cred_interval_99_lc_SA[2],
                   cred_interval_95_lc_SA[1],
                   cred_interval_95_lc_SA[2])

#build dataframe
export_df <- data.frame(labels_column, values_column)
write.csv(export_df, '../R_outputs/sam_lc_SA.csv', row.names = FALSE)
write.csv(signedRankSamples$deltaSamples, '../R_outputs/sam_lc_SA_samples.csv', row.names = FALSE)

...
wo_pre_elev <- scan('../flat_inputs/wo_pre_elev.txt')
wo_post_elev <- scan('../flat_inputs/wo_post_elev.txt')

pre_name <- 'wo_pre_elev'
post_name <- 'wo_post_elev'

wo_elev <- data.frame(wo_pre_elev, wo_post_elev)
names(wo_elev) <- c(pre_name,post_name)
...
set.seed(123456)
random_subsample <- wo_elev[sample(nrow(wo_elev), round(length(wo_pre_elev)*pro_measurements,0)), ]

x <- random_subsample$wo_pre_elev
y <- random_subsample$wo_post_elev
...
bayes_factor_wo_elev <- computeBayesFactorOneZero(signedRankSamples$deltaSamples, 
                                                  whichTest = "Wilcoxon",
                                                  priorParameter = 1 / sqrt(2))
#calculate 95% and 99% HPDI (highest posterior density interval) 
cred_interval_99_wo_elev <- hdi(signedRankSamples$deltaSamples, 0.99)
cred_interval_95_wo_elev <- hdi(signedRankSamples$deltaSamples, 0.95)
...
values_column <- c(posterior_median,
                   bayes_factor_wo_elev,
                   cred_interval_99_wo_elev[1],
                   cred_interval_99_wo_elev[2],
                   cred_interval_95_wo_elev[1],
                   cred_interval_95_wo_elev[2])

#build dataframe
export_df <- data.frame(labels_column, values_column)
write.csv(export_df, '../R_outputs/sam_wo_elev.csv', row.names = FALSE)
write.csv(signedRankSamples$deltaSamples, '../R_outputs/sam_wo_elev_samples.csv', row.names = FALSE)
